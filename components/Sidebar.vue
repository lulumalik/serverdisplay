<template>
    <div class="px-6 py-4 shadow-md bg-white">
        <button v-show="(r.name == 'Account' || r.name == 'Monitoring') && getUser() && getUser().role.name != 'Admin' ? false : true" @click="toRoute(r)" v-for="(r , i) in routes" :key="i" :class="$route.name == r.pathname ? 'bg-sky-500 rounded text-white' : ''" class="mt-4 px-2 w-44 cursor-pointer py-1 rounded flex space-x-3 items-center font-semibold">
